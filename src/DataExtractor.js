...
const DataExtractor = () => {
  const [queries, setQueries] = useState([
    { id: 1, query: 'how to raise kids with a heart for the lord', results: [] }
  ]);
  const [filename, setFilename] = useState('Book_Ephesians_For_You');
  ...
  const addQuery = () => {
    setQueries([...queries, { id: Date.now(), query: '', results: [] }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updatedQueries = await Promise.all(queries.map(async (queryObj) => {
        ...
        if (!Array.isArray(data) || data.length === 0) {
          throw new Error('No data received or data is not in the expected format');
        }
        ...
        return { ...queryObj, results: extractedData };
      }));

      setQueries(updatedQueries);
      setCurrentPublisher(updatedQueries[0].results[0].publisher);
    } catch (error) {
  ...
  const exportScores = () => {
    const scoresData = queries.map(queryObj => {
  ...
  const customTooltip = (context) => {
    console.log('Custom tooltip function called', context);

    const tooltipModel = context.tooltip;
    
    // Only show tooltip if there are dataPoints
    ...
    if (!tooltipEl) {
      ...
      console.log('Created new tooltip element');
    }
    ...
    if (tooltipModel.body) {
      ...
      bodyLines.forEach(function(body, i) {
        ...
        innerHtml += '<div class="tooltip-body">' + 
          ...
          '<span style="color: ' + lineColor + '; background-color: ' + backgroundColorRgb + '; font-weight: bold; padding: 2px 4px; border-radius: 4px;">' + score + '</span>' +
          '</div>';
      });
    ...
    // Adjust tooltip position if it goes off the chart area
    ...
    // Ensure the tooltip doesn't go off the page
  ...
  const chartData = useMemo(() => {
    return {
      datasets: queries.map((queryObj, index) => {
  ...
  const chartOptions = {
    ...
    scales: {
        ...
        y: {
          ...
          display: false,
          min: 0,
          max: highScore * 1.05, // Add 5% to prevent cutting off the top point
          ticks: {
            display: false
          },
          grid: {
      ...
      elements: {
        ...
        point: {
          radius: function(context) {
            ...
            return data[index].y === maxScore ? 8 : 2;
          },
          hoverRadius: function(context) {
            ...
            const data = context.dataset.data;
            const maxScore = Math.max(...data.map(d => d.y));
            return data[index].y === maxScore ? 10 : 4;
          },
          borderColor: function(context) {
            ...
            const data = context.dataset.data;
            const maxScore = Math.max(...data.map(d => d.y));
            return data[index].y === maxScore ? 'white' : context.dataset.borderColor;
          },
          ...
          borderWidth: function(context) {
            ...
            const data = context.dataset.data;
            const maxScore = Math.max(...data.map(d => d.y));
            return data[index].y === maxScore ? 4 : 0;
          },
        }
      },
    ...
    return (
      <div className="container" style={{ maxWidth: `${graphWidth + 80}px` }}>
        ...
        <form onSubmit={handleSubmit} className="form">
          ...
          <div className="input-group">
            <input
              ...
              placeholder="Book_Ephesians_For_You"
              required
            />
        ...
        {queries.some(q => q.results.length > 0) && (
